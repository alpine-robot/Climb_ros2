...
class AlpineOdometryNode(Node):
    ...
    Uses:
      ...
      - /alpine/dongle/telemetry = [epoch_ms, imu1[11], imu2[11]]
    ...
    Debug layout:
      ...
      [1] l1_eff
      [2] l2_eff
      [3:6] left_attachment xyz
      [6:9] right_attachment xyz
      [9:12] body_pos xyz
      [12] attachment_dist
      [13] attachment_dist_error
      [14] right_anchor_error
      [15] modeled_l2_error_signed
    """
    ...
    def __init__(self):
        ...
        # Winch anchors in world
        self.declare_parameter('anchor_left_xyz', [0.0, 0.0, 0.0])
        self.declare_parameter('anchor_right_xyz', [1.30, 0.0, 0.0])

        # Geometry in BODY frame
        self.declare_parameter('right_attachment_from_left_body_xyz', [-0.55, 0.0, 0.0])
        self.declare_parameter('body_origin_from_left_attachment_xyz', [-0.275, 0.0, 0.0])

        # Rope homing offsets
        self.declare_parameter('left_home_offset_m', 0.0)
        self.declare_parameter('right_home_offset_m', 5.05)

        # Axis of imu2 aligned with the left rope
        ...
        self.right_attachment_from_left_body = np.array(
            self.get_parameter('right_attachment_from_left_body_xyz').value, dtype=float
        )
        self.body_origin_from_left_attachment = np.array(
            self.get_parameter('body_origin_from_left_attachment_xyz').value, dtype=float
        )
    ...
    def _update(self):
        ...
        if self.tf_broadcaster is not None:
            ...
            t.transform.translation.x = float(body_pos[0])
            t.transform.translation.y = float(body_pos[1])
            t.transform.translation.z = float(body_pos[2])
